...
double GetDifficulty(const CBlockIndex*);
...
{
    // Faircoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetSpacing = 2.5 * 60; // Faircoin: 2.5 minute blocks
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Faircoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Faircoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool CBlock::AcceptBlock(CValidationState &state, CDiskBlockPos *dbp)
{
    ...
    if (hash != hashGenesisBlock) {
        ...
        // Check reward TODO
        //if (nBits != GetNextWorkRequired(pindexPrev, this))
        //    return state.DoS(100, error("AcceptBlock() : incorrect reward value"));
...
{
    // Faircoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "THE INDEPENDENT - MONDAY 13 JANUARY 2014 - NSA’s top-secret cyber squad: The hackers at the Office of Tailored Operations are one of the strongest weapons in America's formidable arsenal";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = GetDifficulty(0) * COIN;
        ...
        assert(block.hashMerkleRoot == uint256("0x1f54046cb49689a1ae9a266fcdc2bfb6972ea0313c49bec4a883d02e04ccd6c4"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Faircoin: increase each by adding 2 to bitcoin's value.
...
void static FaircoinMiner(CWallet *pwallet)
{
    printf("FaircoinMiner started\n");
    ...
    {
        printf("FaircoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&FaircoinMiner, pwallet));
}
